...
int main( int argc, char *argv[] ) {
  ...
  if ( myPart == CLIENT ) {
    ...
    case 3:
      ...
      for (int n = 0; n <= 10; n++){// for the percentages
    	    timer.start( );                                        // start timer
 	        retransmits =
  ...
  if ( myPart == SERVER ) {
    ...
    case 3:
      ...
      int windowSize = 1;
      for (int n = 0; n <= 10; n++){
        serverEarlyRetrans( sock, MAX, message, windowSize, n );
      }
      }
      break;  
...
void serverUnreliable( UdpSocket &sock, const int max, int message[] ) {
  ...
  for ( i = 0; i < max; i++ ) {
    ...
    cerr << "message: " << i << " value: " << message[0] << endl;                     // print out message
  }
  
  cout << "done with serverUnreliable, i is: " << i << endl;
